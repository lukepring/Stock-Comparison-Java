# Meeting Report — 18/02/2026 (13:00–14:00)

## Project
**Stock Comparison Application**

## Meeting Goal
- Plan Sprint 2 implementation tasks
- Split responsibilities (backend/frontend)
- Update Kanban board with Sprint 2 backlog + priorities

## Roles
- **Facilitator:** Luke Pring  
- **Note taker:** Sameer Shabbir  

## Attendees
- Luke Pring  
- Jack Turner  
- Azlan Rashid  
- Sameer Shabbir  

## Roundtable Updates
- **Luke:** Planned Sprint 2 priorities and task split.  
- **Jack:** Will contribute to frontend/UI implementation.  
- **Azlan:** Updated the architecture plan for Sprint 2 features.  
- **Sameer:** Continued meeting reports and tracked tasks/updates on the board.  

## Discussion points
- Planned Sprint 2 build tasks and priorities
- Discussed offline support using stored/cached data
- Confirmed approach for retrieving share prices 
- Assigned backend vs frontend responsibilities and updated Kanban board

## Actions
- Add Sprint 2 tasks + priorities to the Kanban board (**Luke + Jack**)
- Start Sprint 2 coding (**Sameer + Jack**)
- Update the architecture diagram for Sprint 2 (**Azlan**)
- Support team with implementation tasks (**All team members**)
