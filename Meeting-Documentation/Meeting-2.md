# Meeting Report — 16/02/2026 (13:00–13:45)

## Project
**Stock Comaprison Application**

## Meeting Goal
- Review Sprint 1 deliverables against the mark scheme
- Demo the Java project builds and match architecture/diagram
- Collect issues/feedback and convert them into Sprint 2 tasks

## Roles
- **Facilitator:** Luke Pring  
- **Note taker:** Sameer Shabbir  

## Attendees
- Luke Pring  
- Jack Turner  
- Azlan Rashid  
- Sameer Shabbir  

## Roundtable Updates
- **Luke:** Checked Sprint 1 work matches the marking criteria.  
- **Jack:** Reviewed repository governance and contribution guidelines.  
- **Azlan:** Presented the architecture diagram and explained it.  
- **Sameer:** Completed Sprint 1 meeting reports and organised evidence for submission.  

## Discussion points
- Reviewed Sprint 1 deliverables (requirements, architecture, diagram, code)
- Checked repo evidence: branches, commits, project board, Code of Conduct
- Noted improvements needed for Sprint 2 (real API fetch, storage)
- Created Sprint 2 backlog tasks from feedback

## Actions
- Review Sprint 1 for code review + fix any issues (**Luke + Sameer**)
- Support team with repository processes (**Jack + Azlan**)
- Final check of architecture diagram to ensure it matches code structure (**Azlan**)
- Update Kanban with Sprint 2 tasks from feedback (**Luke + Jack**)
